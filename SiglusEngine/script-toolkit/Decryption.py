...
try:
    ...
    # DLL not available, basic Decrypt function still works
    DLL_AVAILABLE = False
    print("提示: Decryption.dll 未找到。")
    print("      - 基础字符串加密/解密功能 (Decrypt) 仍然可用")
    print("      - ssTextExtractor.py 和 ssTextPacker.py 可以正常使用")
    print("      - siglus_decrypt_unpack.py 的解密/解包功能仍然可用")
    print("      - 高级解密功能 (Decrypt1-5, Compress 等) 已禁用")
