...
def rebuild_s4(yks, trans, out_enc='cp932'):
    """S4重建: 混合策略 (inplace + append fallback)
    
    许多Yuka引擎会在加载时把字符串按原S4位置索引化/缓存,
    纯append策略会导致修改不生效。
    
    策略:
      1. 新字节数 ≤ 原字节数 → 原位覆盖 (inplace), s2.f2 不变
         - 剩余字节用 NUL 填充 (引擎遇NUL停止,不影响显示)
      2. 新字节数 > 原字节数 → 退回 append 模式
         - 这种情况通常极少 (翻译后比原文长才会触发)
    
    原位覆盖方式和原S4布局完全兼容,引擎无论走s2.f2还是走缓存
    都能读到新内容。append仅作为不得已的回退。
    """
    all_refs = yks.get_all_s4_refs()
    new_s4 = bytearray(yks.s4)
    inplace_count = 0
    append_count = 0
    skipped = 0
    append_warnings = []
    ...
    for s2i, new_text in trans.items():
        ...
        try:
            ...
            print(f"[warn] S2[{s2i}] encode fail: {e}")
            continue

        # 计算原字符串占位 (含末尾NUL)
        old_end = new_s4.find(b'\x00', f2)
        if old_end < 0:
            old_end = len(new_s4) - 1
        old_slot_size = old_end - f2 + 1  # 原占位字节数 (含NUL)
        old_raw = bytes(new_s4[f2:old_end + 1])

        if new_raw == old_raw:
            skipped += 1
            continue

        if len(new_raw) <= old_slot_size:
            # 原位覆盖: 新内容 + NUL终止, 剩余字节填NUL
            new_slot = new_raw + b'\x00' * (old_slot_size - len(new_raw))
            new_s4[f2:f2 + old_slot_size] = new_slot
            # s2.f2 不变
            inplace_count += 1
        else:
            # append 回退
            ...
            append_count += 1
            if len(append_warnings) < 5:
                append_warnings.append((s2i, old_slot_size, len(new_raw), new_text[:40]))

    yks.s4 = bytes(new_s4)
    if append_warnings:
        print(f"[info] {append_count} 条翻译比原文长,走append回退:")
        for s2i, orig, new, txt in append_warnings:
            print(f"  s2[{s2i}] 原{orig}B → 新{new}B: {txt!r}")
        if append_count > 5:
            print(f"  ...等共 {append_count} 条")
    
    return inplace_count + append_count, inplace_count + append_count, inplace_count, append_count
...
def do_inject(args):
    ...
    _, _, inplace, append = rebuild_s4(yks, trans, args.encoding)
    out = args.output or os.path.splitext(args.input)[0] + '_cn.yks'
    yks.save(out, encrypt=args.encrypt)
    print(f"[inject] 原位替换{inplace}条 append{append}条 → {os.path.basename(out)}")
