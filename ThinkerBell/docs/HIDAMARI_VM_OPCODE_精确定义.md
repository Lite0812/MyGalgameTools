# HIDAMARI VM Opcode 精确定义

本文按 `Opcode | 命名 | 长度/格式 | 精确定义 | 佐证` 五列表整理 HIDAMARI `.a` 脚本 VM 的顶层 opcode。分析对象来自 [export-for-ai/decompile/](export-for-ai/decompile/) 与现有 [arc02/](arc02/) 样本；核心分发器为 [4239C0.c](export-for-ai/decompile/4239C0.c)。
...
| `60` | `NOOP_60_TEXT_PAYLOAD` | `M#N 60, raw S...`；样本长度 9/28/34 | dispatcher 调 `nullsub_9`，不消费参数；样本中 opcode 头后直接跟 `S...` 文本字节，实际作为保留/注释型 payload。 | [4239C0.c](export-for-ai/decompile/4239C0.c)；[arc02/](arc02/) |
